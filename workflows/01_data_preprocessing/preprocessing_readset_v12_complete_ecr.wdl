...
workflow CompletePreprocessingWorkflowV12 {
    meta {
        description: "完整的测序数据预处理工作流v12 - 使用私有ECR镜像，包含所有预处理步骤"
        version: "12.0"
...
task RemoveDuplicates {
    ...
    command <<<
        ...
        # 计算去重复率
        ORIGINAL_COUNT=$(seqkit stats ~{fastq_file} | tail -n 1 | awk '{print $4}' | sed 's/,//g')
        DEDUP_COUNT=$(seqkit stats ~{sample_name}_dedup.fastq.gz | tail -n 1 | awk '{print $4}' | sed 's/,//g')
        
        if [ "$ORIGINAL_COUNT" -gt 0 ]; then
            DUPLICATE_RATE=$(echo "scale=2; ($ORIGINAL_COUNT - $DEDUP_COUNT) * 100 / $ORIGINAL_COUNT" | bc -l)
            echo "重复序列数量: $(($ORIGINAL_COUNT - $DEDUP_COUNT))" >> ~{sample_name}_dedup_stats.txt
