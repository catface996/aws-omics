...
## 最终修正版 Read Set 预处理工作流
## 使用正确的ECR镜像URI格式

workflow PreprocessingWorkflowReadSet {
    meta {
        description: "最终修正版测序数据预处理工作流"
        version: "2.2"
...
task RunFastQC {
    ...
    command <<<
        ...
        # 创建输出目录
        mkdir -p /tmp/fastqc_output
        
        # 运行FastQC
        /tmp/FastQC/fastqc \
            --outdir /tmp/fastqc_output \
            --threads 2 \
            --format fastq \
            ~{fastq_file}
        
        # 重命名输出文件
        mv /tmp/fastqc_output/*.html ~{sample_name}_fastqc.html
        mv /tmp/fastqc_output/*.zip ~{sample_name}_fastqc.zip
    >>>
